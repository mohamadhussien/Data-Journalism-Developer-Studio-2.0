...
# open a browser to get the search string
print {*STDERR} 'Enter Twitter Search string or just \'enter\' to open Advanced Search in a browser: ' or
  croak $OS_ERROR;
my $search_string = ''; $search_string = <>; chomp $search_string;

if (length $search_string < 1) {
  my $browser = Browser::Open::open_browser('http://search.twitter.com/advanced');
  print {*STDERR} "Paste the search string here and 'enter' when ready\n" or croak $OS_ERROR;
...
YAML::XS::DumpFile('headers.yaml', \%object_keys);
exit;
